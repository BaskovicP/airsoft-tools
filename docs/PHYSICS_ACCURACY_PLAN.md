...
1. Corrected piston static friction and contact settlement so the complete drive is balanced and low velocity alone cannot mean “settled.”
2. Added a conservative vented control volume ahead of the BB; BB thrust now uses behind-minus-ahead pressure.
3. Made minimum-step and step-budget exhaustion explicit invalid convergence outcomes.
4. Added a causal acoustic travel-time/pressure-history envelope and graph trace, while retaining the lumped solver and disclosing the missing coupled 1D solution.
5. Changed optimization from first-strike-only ranking to total completed contact loss and added wave-timing uncertainty as a fifth objective.
6. Extended calibration to one–three selected parameters and optional contact-time, peak-pressure and peak-bumper-force observations, with minimum distinct-setup and measured-law guards.
7. Added measured nonlinear bumper force–deflection input, conservative elastic-energy integration and UI/optimizer/calibration protection for that measured law.
