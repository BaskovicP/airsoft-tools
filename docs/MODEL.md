...
Front contact uses a dissipative restitution coefficient. Repeated contacts remain possible. The primary sound-comparison quantity is still the **first** contact, while the contact-sequence panels in Graphs and Results report every resolved front-boundary contact separately. Thickness changes contact travel, swept volume, receiving-passage length and contact spring compression; thickness together with the independent inner diameter sets annular occupied volume and local pin clearance. Restitution changes only the instantaneous post-contact velocity. This is still not a resolved rubber spring/damper or prediction of compression, hardness, peak contact force, contact duration, sound or durability. The animation's contact bulge is a symbolic state cue.
...
Each `pistonImpacts` entry stores exact solver time, incoming and post-contact velocity, piston-only kinetic energy, effective-moving-mass energy, dissipated contact energy and the two modeled gas pressures at contact. Collection occurs at the boundary event and is independent of display sampling. A pressure-driven reversal that never reaches the contact boundary is not an impact. The UI plots every entry in an expanded contact-time window and lists exact values for the first eight; the exported run retains all events. Summing these energies does not produce peak force, audible strike count, acoustic energy, loudness or dB because contact duration, bumper deformation and structural acoustics are unresolved.
