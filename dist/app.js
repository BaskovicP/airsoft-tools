...
(() => {
  ...
  function render() {
    ...
    bindLanguage(); bindControls(); bindFieldHelp(); bindOptimizer(); syncSpringControls(); syncBumperControls(); updateResults(); renderMeasurements(); renderOptimizerResults(); workspace.restoreScroll();
  }
  ...
  function optimizerMarkup() {
    ...
    return `<section class="panel optimizer"><div class="optimizer-heading"><div><h2>${t("Find a quieter, efficient setup", "Pronađi tišu i učinkovitiju konfiguraciju")}</h2><p class="small-note">${t("Freeze the parts you want to keep. Search only the hardware choices you can change.", "Zamrznite dijelove koje želite zadržati. Pretražujte samo zamjenjive dijelove.")}</p></div><button class="primary-button" type="button" id="toggleOptimizer" aria-expanded="${optimizer.open}" aria-controls="optimizerBody">${t("Freeze parts & optimize", "Zamrzni dijelove i optimiziraj")}</button></div>
      ...
      <p class="small-note">${t("Weather, fitted airflow/leakage, friction, rear damping, rigid restitution and timing criteria stay fixed. Bumper stiffness/damping/compression are searchable hardware only when the head group is unlocked. Every candidate uses the same 250 ms observation limit. Rankings compare total modeled contact dissipation across all contacts, peak muzzle airflow, exit pressure, efficiency and pressure-wave uncertainty—not dB or guaranteed loudness.", "Vrijeme, prilagođeni protok/curenje, trenje, stražnje prigušenje, kruti odskok i vremenski kriteriji ostaju fiksni. Krutost/prigušenje/stlačenje gumice pretraživi su hardver samo kada je grupa glave otključana. Svaki kandidat koristi isti prozor promatranja od 250 ms. Rangiranje uspoređuje ukupno modelirano rasipanje kroz sve kontakte, vršni protok, izlazni tlak, učinkovitost i nesigurnost tlačnih valova — ne dB ni zajamčenu glasnoću.")}</p>
  ...
  function renderOptimizerResults() {
    ...
    $("optimizerResults").innerHTML = `<p class="optimizer-target">${t("Required model exit energy", "Tražena izlazna energija modela")}: ${fmt(r.bounds.min, 3)}–${fmt(r.bounds.max, 3, "J")}.</p>
      ...
      ${best.length ? `<div class="table-wrap"><table class="optimizer-table"><thead><tr><th>${t("Choice / changes", "Odabir / promjene")}</th><th>${t("BB energy", "Energija BB-a")}</th><th>${t("All-contact loss", "Gubitak svih kontakata")}</th><th>${t("Peak muzzle flow", "Vršni protok")}</th><th>${t("Exit pressure", "Izlazni tlak")}</th><th>${t("Efficiency", "Učinkovitost")}</th><th>${t("Uncertainty", "Nesigurnost")}</th><th></th></tr></thead><tbody>${r.baseline ? `<tr class="optimizer-baseline"><td>${t("Current setup · reference", "Trenutačno · referenca")}</td>${metricCells(r.baseline)}<td></td></tr>` : ""}${best.map((candidate, i) => {
  ...
  function updateResults() {
    ...
    next.innerHTML = `<div class="readout-grid">
      ...
      <p class="small-note">${t("A small numerical residual does not validate the physics. Cylinder, behind-BB and ahead-BB gas are conservative lumped control volumes; profile-sliced duct losses and the effective leaky-piston BB remain approximations. The front volume vents through the muzzle and opposes BB motion. Spatial pressure waves, spring surge, distributed rubber deformation and structural acoustics are not resolved by the primary solver; the causal diagnostic below bounds that limitation without solving it. Bumper force is conditional on the entered force law and damping; sub-millisecond timing still needs instrumented validation.", "Malo numeričko odstupanje ne potvrđuje fizikalni model. Plin u cilindru, iza BB-a i ispred BB-a predstavljaju konzervativne koncentrirane kontrolne volumene; gubici kanala podijeljeni po profilu i BB kao efektivni propusni klip ostaju aproksimacije. Prednji volumen odzračuje kroz usta i suprotstavlja se gibanju BB-a. Primarni solver ne razrješava prostorne tlačne valove, valove opruge, raspodijeljenu deformaciju gume ni strukturnu akustiku; uzročna dijagnostika niže ograničava taj nedostatak bez njegova rješavanja. Sila gumice ovisi o unesenom zakonu sile i prigušenju; vremenski odnos ispod milisekunde i dalje traži instrumentiranu provjeru.")}</p>
  ...
  function syncSetupControls() {
    ...
    syncSpringControls(); syncBumperControls();
  }
  ...
  function bindControls() {
    ...
    document.querySelectorAll("[data-number], [data-range]").forEach(input => input.addEventListener("input", () => {
      ...
      else if (["cylinderBore", "strokeLength", "barrelLength", "barrelDiameter", "frontDeadVolume", "pistonMass", "bbMass", "bbDiameter", "headBore", "headLength", "nozzleBore", "nozzleLength", "bumperThickness", "bumperBore", "bumperStiffness", "bumperDamping", "bumperMaxCompression", "restitution", "airbrakeLength", "airbrakeDiameter", "airbrakeTipDiameter", "airbrakeTaper", "deadVolume", "breechVolume"].includes(key)) provenance.geometry = "assumed";
      document.querySelectorAll("[data-provenance]").forEach(box => { box.checked = provenance[box.dataset.provenance] === "measured"; });
      $("measurementConfirm").checked = false;
      ...
      syncSpringControls(); syncBumperControls(); scheduleCalculation();
    }));
    ...
    $("measurementForm").addEventListener("submit", event => {
      ...
      const row = C.cleanRecord({ id: `shot-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`, bbMass: mass, fps: Number($("measurementFps").value), sigma: Number($("measurementSigma").value),
        pistonHitMs: $("measurementHitMs").value, peakCylinderBarG: $("measurementPressure").value, peakBumperForceN: $("measurementBumperForce").value,
  ...
  function renderMeasurements() {
    ...
    $("measurementRows").innerHTML = measurements.map((r, i) => `<tr><td>${fmt(r.bbMass, 2, "g")}</td><td>${fmt(r.fps, 1)} / ${fmt(C.energy(r.bbMass, r.fps), 3)}${Number.isFinite(r.pistonHitMs) ? `<br><small>${t("contact", "kontakt")} ${fmt(r.pistonHitMs, 2, "ms")}</small>` : ""}${Number.isFinite(r.peakCylinderBarG) ? `<br><small>${t("peak pressure", "vršni tlak")} ${fmt(r.peakCylinderBarG, 2, "bar(g)")}</small>` : ""}${Number.isFinite(r.peakBumperForceN) ? `<br><small>${t("peak bumper force", "vršna sila gumice")} ${fmt(r.peakBumperForceN, 1, "N")}</small>` : ""}</td><td>${r.role === "train" ? t("Training", "Prilagodba") : r.role === "validation" ? t("Held-out", "Neovisna provjera") : t("Reference only", "Samo referenca")}<br><small>${C.eligible(r) ? t("Measured inputs confirmed", "Izmjereni ulazi potvrđeni") : t("Not fit-eligible", "Nije prikladno za prilagodbu")}${r.setup ? ` · ${fmt(r.setup.pistonMass, 1, "g")} / ${fmt(r.setup.barrelLength, 0, "mm")}` : ` · ${t("setup incomplete", "nepotpuna konfiguracija")}`}</small></td><td>${esc(r.notes)}</td><td><button type="button" class="danger-button" data-remove="${i}" aria-label="${t("Remove shot", "Ukloni hitac")} ${i + 1}">×</button></td></tr>`).join("");
  ...
  function bindResults() {
    ...
    results.onclick = event => {
      ...
      else if (button.id === "exportRun") download("airsoft-pneumatic-run.json", { schemaVersion: 3, solverVersion: P.VERSION, identity: { selectedPlatform, component, springLabel, pinLabel }, provenance, setup: p, result: shot, baseline: { exitEnergy: baseline?.exitEnergy ?? null, impactEnergy: baseline?.impactEnergy ?? null }, limitations: "Unvalidated three-control-volume lumped model with a causal wave-travel envelope; not a coupled 1D flow solution and not exact joules or dB; see docs/MODEL.md" });
    };
  ...
  function drawCharts(time) {
    const traces = [
      ["pressureChart", [[f => (f.cylinderPressure - shot.ambientPressure) / 1e5, "#ffbf69"], [f => (f.pressure - shot.ambientPressure) / 1e5, "#5de4e7"], [f => (f.frontPressure - shot.ambientPressure) / 1e5, "#b99cff"], [f => (f.wavePressureEstimate - shot.ambientPressure) / 1e5, "#8da2ff", [5, 4]]]],
